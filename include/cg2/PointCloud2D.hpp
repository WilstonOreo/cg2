...
namespace cg2
{
  ...
  public:
    ...
    friend mathvector<T> operator+(mathvector<T> const &lhs, mathvector<T> const &rhs)
    {
      mathvector<T> temp(lhs);
      temp += rhs;
      return temp;
    }
    friend mathvector<T> operator-(mathvector<T> const &lhs, mathvector<T> const &rhs)
    ...
    friend mathvector<T> operator*(mathvector<T> const &lhs, double rhs)
    {
      mathvector<T> temp(lhs);
      temp *= rhs;
      return temp;
  ...
  public:
    ...
    TBD_PROPERTY(size_t,width);
    TBD_PROPERTY(size_t,height);
  private:
