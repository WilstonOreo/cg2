					  ...
					  typename vector<T>::size_type k, Compare __comp = std::less<T>())
{
	...
	for (; it != e; ++it)
	{
		...
		if (__comp(*it,pivot)) less.push_back(*it); else
						   	   greater.push_back(*it);	
	}

	size_t q = less.size();
	if (q > k && !less.empty())   	return quick_select(less,less.begin(),less.end(),k,__comp);
	if (q < k && !greater.empty()) 	return quick_select(greater,greater.begin(),greater.end(),k-q-1,__comp);
	return pivot;
}
