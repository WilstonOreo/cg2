...
namespace cg2
{
  ...
  struct Coords3f
  {
    Coords3f( float _x = 0.0f, float _y = 0.0f, float _z = 0.0f) { set(_x,_y,_z); }
  ...
  struct Vec3f : public Coords3f
  {
    ...
    void operator += ( Vec3f v ) { x += v.x; y += v.y; z += v.z; }
    void operator -= ( Vec3f v ) { x -= v.x; y -= v.y; z -= v.z; }
  };
  ...
  struct Point3f : public Coords3f
  {
    ...
    friend Vec3f operator-( const Point3f& a, const Point3f& b) { return Vec3f(a.x-b.x, a.y-b.y, a.z-b.z); }
    friend Point3f operator+( const Point3f& a, const Point3f& b) { return Point3f(a.x+b.x, a.y+b.y, a.z+b.z); }
    friend Point3f operator+( const Point3f& a, const Vec3f& b) { return Point3f(a.x+b.x, a.y+b.y, a.z+b.z); }

    Vec3f vec3f() { return Vec3f(x,y,z); }
  };
